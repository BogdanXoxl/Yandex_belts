...
public:
    ...
    auto end() const;
private:
    vector<Page<Iterator>> _v;
};

template<typename Iterator>
Paginator<Iterator>::Paginator(Iterator begin, Iterator end, size_t page_size) {
    for(size_t left = distance(begin, end); left ;){
        size_t current_size = min(left, page_size);
        _v.push_back({begin, next(begin, current_size)});
        begin = next(begin, current_size);
        left -= current_size;
    }
}
